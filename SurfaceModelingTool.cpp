...
Standard_Integer SurfaceModelingTool::Arrange_Coons_G0(std::vector<Handle(Geom_BSplineCurve)>& curveArray, Handle(Geom_BSplineCurve)& bslpineCurve1, Handle(Geom_BSplineCurve)& bslpineCurve2, Handle(Geom_BSplineCurve)& bslpineCurve3, Handle(Geom_BSplineCurve)& bslpineCurve4, Standard_Real Tol, Standard_Integer IsModify)
{
	//Standard_Real Tol = 2;
	...
	std::vector<Handle(Geom_BSplineCurve)>& vISOcurvesArray)
{
	...
	Standard_Real maxDist = RealFirst();
...
Standard_Real CompareCurve(const std::vector<gp_Pnt>& points, std::vector<Standard_Real>& params, Handle(Geom_BSplineCurve)& BsplineCurve
	, std::vector<Standard_Real>& DisCovParams, Standard_Boolean isSingle, Standard_Real Toler)
{
	...
	Standard_Real maxError1 = 0;
	Standard_Real maxParam = 0;
	Standard_Boolean NeedIter = Standard_False;
	for (Standard_Integer i = 0; i < params.size(); i++) {
		gp_Pnt pnt = BsplineCurve->Value(params[i]);
		Standard_Real x, y, z;
		pnt.Coord(x, y, z);
		Standard_Real disTemp = pnt.Distance(points[i]);
...
Standard_Integer FindSpan(Standard_Integer n, Standard_Integer p, Standard_Real u, const std::vector<Standard_Real>& Knots)
{
	if (u == Knots[n + 1])
		return n;
...
std::vector<Standard_Real> KnotGernerationByMergeKnots(const std::vector<Standard_Real>& KnotsA, const std::vector<Standard_Real>& KnotsB)
{
	std::vector<Standard_Real> C;
...
Standard_Real OneBasicFun(Standard_Real u, Standard_Integer i, Standard_Integer p, std::vector<Standard_Real>& Knots)
{
...
gp_Vec CalResPnt(Standard_Integer k, const std::vector<gp_Pnt>& dataPoints, std::vector<Standard_Real>& parameters, Standard_Integer p,
...
void ComputeChordLength(const std::vector<gp_Pnt>& points, std::vector<Standard_Real>& parameters)
{
...
//CtrlPntNum n
Handle(Geom_BSplineCurve) ApproximateC(const std::vector<gp_Pnt>& Pnts, std::vector<Standard_Real>& PntParams, TColStd_Array1OfReal& Knots, TColStd_Array1OfInteger& Mutis, std::vector<Standard_Real>& FKnots,
...
//To approximate a Bspline curve by a set of points
	...
	//Assume there are m data points.We need to use n( CtrlPntNum)+1 to approximate the m data points .
	//CtrlPntNum n
Handle(Geom_BSplineCurve) ApproximateC(const std::vector<gp_Pnt>& Pnts, std::vector<Standard_Real>& params, std::vector<Standard_Real>& FKnots, Standard_Integer degree)
{
...
void SurfaceModelingTool::CreateLoftingSurface(const std::vector<Handle(Geom_BSplineCurve)>& curvesArray, const std::vector<gp_Vec>& normals, std::vector<TopoDS_Shape>& loftingSurfaces, std::vector<Handle(Geom_BSplineCurve)>& theLoftingCurves)
{
	Standard_Real offsetDistance = 400;
	for (Standard_Integer i = 1; i < curvesArray.size() - 1; i++)
	{
		...
		theLoftingCurves.push_back(aBSplineCurve1);
		...
		theLoftingCurves.push_back(aBSplineCurve2);
...
Standard_Real CalPointsChordLen(const std::vector<gp_Pnt>& points)
{
	if (points.size() < 2) {
		return 0.0;
	}
...
gp_Vec CalTangent(const std::vector<gp_Pnt>& points, Standard_Real TanMagnitude)
{
	if (points.size() != 3) {
	...
	Standard_Real magnitude = tangentVec.Magnitude();
	if (magnitude == 0.0) {
	...
	Handle(Geom_BSplineSurface) CoonsSurface)
{
	...
	for (Standard_Integer i = 1; i < LoftingSur.size() - 1; i++)
	{
		...
		if (aInterPnts.size() > 0)
		{
			...
			gp_Vec FirstD1 = startDirection;
			gp_Vec LastD1 = endDirection;
			...
			if (!CoonsSurface.IsNull())
			{
				...
				if (projector.NbPoints() > 0)
				{
					...
					Standard_Real projectionMagnitude = startDirection.Dot(normalVector) / normalVector.Magnitude();
			...
			if (!CoonsSurface.IsNull())
			{
				...
				if (projector.NbPoints() > 0)
				{
					...
					Standard_Real projectionMagnitude = endDirection.Dot(normalVector) / normalVector.Magnitude();
					gp_Vec projection = normalVector * projectionMagnitude; // 计算投影向量
			...
			if (ISOcurvesArray_Initial[0]->StartPoint().Distance(ISOcurvesArray_Initial[ISOcurvesArray_Initial.size() - 1]->StartPoint()) < 10.0)
			{
				gp_Pnt aTempPnt;
				gp_Vec InterpolateD1;
				gp_Vec InterpolateD2;
				ISOcurvesArray_Initial[0]->D1(ISOcurvesArray_Initial[0]->FirstParameter(), aTempPnt, InterpolateD1);
				ISOcurvesArray_Initial[ISOcurvesArray_Initial.size() - 1]->D1(ISOcurvesArray_Initial[ISOcurvesArray_Initial.size() - 1]->FirstParameter(), aTempPnt, InterpolateD2);
				Standard_Real weight = (Standard_Real)i / isoCount;
				FirstD1 = weight * InterpolateD2 + (1 - weight) * InterpolateD1;
				FirstD1.Multiply(CalPointsChordLen(aPntsVector) / FirstD1.Magnitude());
				TangentArray1.push_back(BRepBuilderAPI_MakeEdge(aTempPnt, aTempPnt.Translated(FirstD1 * 0.1)).Edge());
			}
			...
			TColgp_Array1OfVec tangents(1, aPntsVector.size());  // 4个切向量
			tangents(1) = FirstD1;  // 起点切向量          
			Handle(TColStd_HArray1OfBoolean) tangentFlags = new TColStd_HArray1OfBoolean(1, aPntsVector.size(), Standard_False);
			tangentFlags->SetValue(1, Standard_True);

			interpolate.Load(tangents, tangentFlags, Standard_True);  // 加载切向量
			// 执行插值计算
			interpolate.Perform();
			// 检查是否成功完成插值
...
std::vector<Standard_Real> GetKnotsSequence(Handle(Geom_BSplineCurve) curve)
{
	std::vector<Standard_Real> debugKnots;
...
gp_Vec CalResPnt(Standard_Integer k, const std::vector<gp_Pnt>& dataPoints, const gp_Pnt& SecondPoint, const gp_Pnt& LastSecondPoint, const std::vector<Standard_Real>& parameters, Standard_Integer p,
	std::vector<Standard_Real>& Knots, Standard_Integer CtrlPntNum) {
...
Handle(Geom_BSplineCurve) EndDerivaConstraintBsplineCurveAppro(const std::vector<gp_Pnt>& Pnts, const gp_Vec& FirstD1, const gp_Vec& LastD1,
	std::vector<Standard_Real>& Params, std::vector<Standard_Real>& KnotSequences, Standard_Integer degree) {
	...
	gp_Vec second_point_vectype = first_point_vectype + (KnotSequences[degree + 1] / (Standard_Real)degree) * FirstD1;
	...
	gp_Vec last_second_point_vectype = last_point_vectype - ((1 - KnotSequences[index]) / (Standard_Real)degree) * LastD1;
...
Handle(Geom_BSplineCurve) IterateApproximate(std::vector<Standard_Real>& InsertKnots,
	...
	std::vector<Standard_Real>& PntsParams, std::vector<Standard_Real>& InitKnots,
	Standard_Integer degree, Standard_Integer MaxIterNum, Standard_Real toler) {
	Standard_Integer itNum = 1;
	Standard_Real currentMaxError = 100;
	Handle(Geom_BSplineCurve) IterBspineCurve;
	std::vector<Standard_Real> CurrentKnots = InitKnots;
...
//To compute the Res Point Value - first d1 case
gp_Vec CalResPnt(Standard_Integer k, const std::vector<gp_Pnt>& dataPoints, const gp_Pnt& SecondPoint, const std::vector<Standard_Real>& parameters, Standard_Integer p,
	std::vector<Standard_Real>& Knots, Standard_Integer CtrlPntNum) {
	Standard_Real aCoeff0 = OneBasicFun(parameters[k], 0, p, Knots);
	Standard_Real aCoeff1 = OneBasicFun(parameters[k], 1, p, Knots);
	Standard_Real aCoeffm = OneBasicFun(parameters[k], CtrlPntNum, p, Knots);
...
std::pair<Standard_Real, Standard_Real> processPoints(const gp_Pnt& P1, const gp_Pnt& P2, const std::vector<gp_Pnt>& isoInterpolatePoints, const std::vector<gp_Pnt>& oppsiteInterpolatePoints)
{
	std::vector<std::pair<Standard_Real, gp_Pnt>> distancesP1;
	for (const auto& point : isoInterpolatePoints)
	{
		distancesP1.push_back({ P1.Distance(point), point });
	}

	std::sort(distancesP1.begin(), distancesP1.end(), [](std::pair<Standard_Real, gp_Pnt> p1, std::pair<Standard_Real, gp_Pnt> p2) {return p1.first < p2.first; });
	...
	// 计算投影
	Standard_Real projectionLength = vecToP1.Dot(lineVec) / lineVec.Magnitude();

	// 判断投影是否在连线区间内
	Standard_Boolean isOppositeSide = Standard_False;
	if (projectionLength >= 0 && projectionLength <= lineVec.Magnitude())
	{
		isOppositeSide = Standard_True;  // 如果投影在连线区间内，认为两点在两侧
	}
	...
	while (!isOppositeSide && next < distancesP1.size())  // 检查索引是否越界
	{
		...
		if (projectionLength >= 0 && projectionLength <= lineVec.Magnitude())
		{
			isOppositeSide = Standard_True;  // 找到满足条件的点
		}
	}

	// 计算P2的最近点
	std::vector<std::pair<Standard_Real, gp_Pnt>> distancesP2;
	for (const auto& point : oppsiteInterpolatePoints)
	{
		distancesP2.push_back({ P2.Distance(point), point });
	}

	std::sort(distancesP2.begin(), distancesP2.end(), [](std::pair<Standard_Real, gp_Pnt> p1, std::pair<Standard_Real, gp_Pnt> p2) {return p1.first < p2.first; });
	...
	// 计算投影
	Standard_Real projectionLength_P2 = vecToP2.Dot(lineVec_P2) / lineVec_P2.Magnitude();  // 归一化投影长度，防止除以线段的长度

	// 判断投影是否在连线区间内（0 <= projectionLength_P2 <= lineVec_P2.Magnitude()）
	Standard_Boolean isOppositeSide_P2 = Standard_False;
	if (projectionLength_P2 >= 0 && projectionLength_P2 <= lineVec_P2.Magnitude())
	{
		isOppositeSide_P2 = Standard_True;  // 如果投影在连线区间内，认为两点在两侧
	}
	...
	while (!isOppositeSide_P2 && nextP2 < distancesP2.size())
	{
		...
		if (projectionLength_P2 >= 0 && projectionLength_P2 <= lineVec_P2.Magnitude())
		{
			isOppositeSide_P2 = Standard_True;  // 找到满足条件的点
		}
	}

	// 计算半径
	Standard_Real L1 = P1.Distance(nearestPoint1_P1) + P1.Distance(nearestPoint2_P1);
	Standard_Real L2 = P2.Distance(nearestPoint1_P2) + P2.Distance(nearestPoint2_P2);
	Standard_Real searchRadius = (std::max(L1, L2) - std::min(L1, L2)) / 2;
...
void ProcessISOCurvesWithTangent(
	...
	std::vector<std::vector<Standard_Real>>& knotsArray,
	...
	const std::vector<Handle(Geom_BSplineSurface)>& surfaceArr)
{
	...
	for (Standard_Integer i = 0; i < isoCurvesArray_New.size(); i++)
	{
		...
		for (Standard_Integer j = 0; j < oppsiteISOcurvesArray_New.size(); j++)
		{
			...
			if (extrema.NbExtrema() > 0)
			{
				...
				std::pair<Standard_Real, Standard_Real> weights = processPoints(P1, P2, isoInterpolatePoints[i], oppsiteInterpolatePoints[j]);
				// 获取权重值
				Standard_Real w1 = weights.first;
				Standard_Real w2 = weights.second;
				gp_Pnt midPoint;
				Standard_Real x = (P1.X() * w1 + P2.X() * w2);
				Standard_Real y = (P1.Y() * w1 + P2.Y() * w2);
				Standard_Real z = (P1.Z() * w1 + P2.Z() * w2);
	...
	// U_Tangent指在该点的U向切向，V_Tangent指在该点的V向切向
		...
		gp_Vec U_EndTangent, V_EndTangent;
		// 计算切向和指定方向的角度，取小的
		Standard_Real cosAngleStartU, cosAngleStartV, cosAngleEndU, cosAngleEndV;
		...
		std::vector<Standard_Real> params = ComputeUniformParam(intersectionPoints.size(), 0., 1.);
		std::vector<Standard_Real> tempKnots = KnotGernerationByParams(params, 3, degree);
		...
		auto areVectorsEqual = [](const gp_Vec& vec1, const gp_Vec& vec2, Standard_Real tol = 1e-6)
			{
	...
	std::vector<Handle(Geom_BSplineSurface)>& surfaceArr)
{
	...
	Standard_Real minDistance = INT_MAX;  // 初始为最大值
...
void SurfaceModelingTool::GetISOCurveWithNormal(const Handle(Geom_BSplineSurface)& surfacecoons,
	...
	std::vector<Handle(Geom_BSplineCurve)>& vISOcurvesArray_Initial, std::vector<gp_Vec>& normalsOfUISOLines, std::vector<gp_Vec>& normalsOfVISOLines,
	std::vector<TopoDS_Edge>& NormalArray, Standard_Integer numIsoCurves)
{
	...
	auto GetSurfaceNormal = [](const Handle(Geom_Surface)& surface, const gp_Pnt& point)
		{
			// 使用GeomAPI_ProjectPointOnSurf来计算点的投影，并获取u,v参数
			GeomAPI_ProjectPointOnSurf projector(point, surface);

			if (!projector.IsDone())
			{
				return gp_Vec(0, 0, 0);  // 失败时返回零向量
			}

			// 获取投影点和其对应的u, v参数
			Standard_Real uParameter, vParameter;
			projector.LowerDistanceParameters(uParameter, vParameter);

			// 计算法向量
			gp_Pnt aTempPnt;
			gp_Vec Du, Dv;
			surface->D1(uParameter, vParameter, aTempPnt, Du, Dv);

			if (Du.Magnitude() == 0 || Dv.Magnitude() == 0) return gp_Vec(0, 0, 0);
			gp_Vec aNormal = Du.Crossed(Dv);
			aNormal.Normalize();
			return aNormal;
		};
	...
	for (Standard_Integer i = 1; i < numIsoCurves; i++)
	{
		...
		Handle(Geom_BSplineCurve) aUGeom_BSplineCurve = Handle(Geom_BSplineCurve)::DownCast(surfacecoons->UIso(((Standard_Real)i / numIsoCurves)
		...
		// 处理v方向的Iso曲线
		Handle(Geom_BSplineCurve) aVGeom_BSplineCurve = Handle(Geom_BSplineCurve)::DownCast(surfacecoons->VIso(((Standard_Real)i / numIsoCurves)
		...
		// 获取u方向BSpline曲线的参数范围
		Standard_Real uStart = aUGeom_BSplineCurve->FirstParameter();
		Standard_Real uEnd = aUGeom_BSplineCurve->LastParameter();
		for (Standard_Integer j = 0; j < numSamplePoints; j++)
		{
			Standard_Real t = uStart + j * (uEnd - uStart) / (numSamplePoints - 1); // 在实际参数范围内均匀取点
			gp_Pnt p1 = aUGeom_BSplineCurve->Value(t);
			gp_Vec N = GetSurfaceNormal(surfacecoons, p1);
			if (N.Magnitude() != 0)
			{
				normalsU.push_back(N);
				// 生成切线边界
				NormalArray.push_back(BRepBuilderAPI_MakeEdge(p1, p1.Translated(N * 100)).Edge());
			}
		}

		gp_Vec avgNormalU = std::accumulate(normalsU.begin(), normalsU.end(), gp_Vec());
		avgNormalU *= 1.0 / normalsU.size();
		normalsOfUISOLines.emplace_back(avgNormalU);
		NormalArray.push_back(BRepBuilderAPI_MakeEdge(aUGeom_BSplineCurve->Value((uEnd + uStart) / 2), aUGeom_BSplineCurve->Value((uEnd + uStart) / 2).Translated(avgNormalU * 1000)).Edge());

		Standard_Real vStart = aVGeom_BSplineCurve->FirstParameter();
		Standard_Real vEnd = aVGeom_BSplineCurve->LastParameter();
		for (Standard_Integer j = 0; j < numSamplePoints; j++)
		{
			Standard_Real t = vStart + j * (vEnd - vStart) / (numSamplePoints - 1);
		...
		gp_Vec avgNormalV = std::accumulate(normalsV.begin(), normalsV.end(), gp_Vec());
		avgNormalV *= 1.0 / normalsV.size();
		normalsOfVISOLines.emplace_back(avgNormalV);
		NormalArray.push_back(BRepBuilderAPI_MakeEdge(aVGeom_BSplineCurve->Value((vEnd + vStart) / 2), aVGeom_BSplineCurve->Value((vEnd + vStart) / 2).Translated(avgNormalV * 1000)).Edge());
	}
}
	...
	std::vector<Handle(Geom_BSplineCurve)>& vISOcurvesArray_Final)
{
	...
	Standard_Boolean isThreeBoundary = Standard_False;
	for (auto theBoundaryCurve : aBoundarycurveArray)
	{
		Standard_Real distance = theBoundaryCurve->StartPoint().Distance(theBoundaryCurve->EndPoint());
		if (distance < 0.1)
		{
			// 三边
			isThreeBoundary = Standard_True;
			break;
		}
	}
	if (isThreeBoundary)
	{
		...
		Standard_Real distance = p2.Distance(p3);
	...
	{
		Standard_Boolean isUBoundaryFirst = MathTool::ComputeCurveCurveDistance(aBoundarycurveArray[0], uCurve) >
	...
	Standard_Boolean isUFirstCloser = uCurveSamplePoint.Distance(MathTool::ComputeAverageSamplePoint(uBoundaryCurve[0], 10)) <
	...
	Standard_Boolean isVFirstCloser = vCurveSamplePoint.Distance(MathTool::ComputeAverageSamplePoint(vBoundaryCurve[0], 10)) <
	...
	if (theBoundaryCurveArray.size() == 4)
	{
		...
		for (auto& anInternalCurve : theInternalBSplineCurves)
		{
			...
			if ((aDistance1 < 10.0 && aDistance3 < 10.0) || (aDistance2 < 10.0 && aDistance4 < 10.0))
			{
				...
				{
					if (aDistance2 >= 10.0 && aDistance4 >= 10.0)
					{
						theVAngleSum += (std::abs(anAngle2) + std::abs(anAngle4)) / 2.0;
						theVInternalCurve.push_back(anInternalPlanarCurve.GetCurve());
					}
	...
	ReferSurfaceType theReferSurfaceType)
{
	if (theReferSurfaceType == ReferSurfaceType::GORDEN_ONE_DIRECTION_GORDEN) {
		...
		// 使用新算法的标志
		Standard_Boolean aUseNewAlgorithm = Standard_True;
		...
		if (theUInternalCurve.size() > theVInternalCurve.size() && theUInternalCurve.size() >= 4) {
			...
			// 如果条件不满足，回退到现有算法
			aUseNewAlgorithm = Standard_False;
			return nullptr;
		}
...
Standard_Boolean PlanarCurve::IsBSplineCurveLinear(const Handle(Geom_BSplineCurve)& theCurve, Standard_Real theTolerance)
{
	...
	for (Standard_Integer i = 2; i <= aNumPoles; ++i)
	{
		...
		// 检查叉积的模长是否在容差范围内
		if (anAngle > theTolerance) return Standard_False;
	}
	// 所有控制点共线
	return Standard_True;
}
...
void MathTool::SortBSplineCurves(std::vector<std::pair<Handle(Geom_BSplineCurve), gp_Vec> >& theCurves, Handle(Geom_BSplineCurve) theReferCurve)
{
	...
	std::sort(theCurves.begin(), theCurves.end(),
		[&](const std::pair<Handle(Geom_BSplineCurve), gp_Vec>& curve1, const std::pair<Handle(Geom_BSplineCurve), gp_Vec>& curve2) -> Standard_Boolean
		{
			// 获取曲线的平均采样点
			gp_Pnt aAvg, bAvg;
			aAvg = MathTool::ComputeAverageSamplePoint(curve1.first, aNumSamples);
			bAvg = MathTool::ComputeAverageSamplePoint(curve2.first, aNumSamples);
...
void MathTool::SortBSplineCurves(std::vector<Handle(Geom_BSplineCurve)>& theCurves, Handle(Geom_BSplineCurve) theReferCurve)
{
	Standard_Integer aNumSamples = 10;
	...
	// 使用 std::sort 对曲线数组进行排序
	std::sort(theCurves.begin(), theCurves.end(),
		[&](const Handle(Geom_BSplineCurve)& curve1, const Handle(Geom_BSplineCurve)& curve2) -> Standard_Boolean
		{
			// 获取曲线的平均采样点
			gp_Pnt aAvg, bAvg;

			aAvg = MathTool::ComputeAverageSamplePoint(curve1, aNumSamples);
			bAvg = MathTool::ComputeAverageSamplePoint(curve2, aNumSamples);
...
void MathTool::ReverseIfNeeded(std::vector<Handle(Geom_BSplineCurve)>& theCurves)
{
	...
	// 获取第一条曲线作为初始参考
	Handle(Geom_BSplineCurve) firstCurve = theCurves[theCurves.size() / 2];
	...
	// 如果是三边的情况还要求以退化点为 StartPoint
	if (theCurves[0]->EndPoint().Distance(theCurves[theCurves.size() - 1]->EndPoint()) < 10.0)
	{
	...
	Standard_Real theToleranceDistance)
{
	for (Standard_Integer i = 0; i < theInternalBSplineCurves.size(); i++)
	{
		...
		Standard_Real aSplitParamArray[2] = { 0 };
		Standard_Integer foundCount = 0;

		// 计算内部曲线与每条边界曲线的距离并排序
		std::vector<std::pair<Standard_Real, Handle(Geom_BSplineCurve)>> aBoundaryCurvesWithDistance;
		for (auto& aBoundaryCurve : theBoundaryCurveArray)
		{
			Standard_Real aDistance = MathTool::ComputeCurveCurveDistance(theInternalBSplineCurves[i], aBoundaryCurve);
			aBoundaryCurvesWithDistance.emplace_back(aDistance, aBoundaryCurve);
		}

		// 按距离从小到大排序
		std::sort(aBoundaryCurvesWithDistance.begin(), aBoundaryCurvesWithDistance.end(),
			[](const auto& a, const auto& b) { return a.first < b.first; });

		if (aBoundaryCurvesWithDistance[0].first >= theToleranceDistance)
		{
			continue;
		}

		// 找到与内部曲线距离最近的两条边界曲线的交点
		for (size_t j = 0; j < 2 && j < aBoundaryCurvesWithDistance.size(); ++j)
		{
			GeomAPI_ExtremaCurveCurve anExtrema(theInternalBSplineCurves[i], aBoundaryCurvesWithDistance[j].second);

			if (anExtrema.NbExtrema() > 0)
			{
				gp_Pnt anInternalPoint, aBoundaryPoint;
				Standard_Real aParamOnCurve;
				anExtrema.NearestPoints(anInternalPoint, aBoundaryPoint);
				anExtrema.LowerDistanceParameters(aSplitParamArray[foundCount], aParamOnCurve);

				replacePoints[foundCount] = aBoundaryPoint;
				foundCount++;
			}
		}
		...
		// 确保裁剪参数按升序排列
		if (aSplitParamArray[0] > aSplitParamArray[1])
		{
			std::swap(aSplitParamArray[0], aSplitParamArray[1]);
			std::swap(replacePoints[0], replacePoints[1]);
		}

		// 裁剪内部曲线并更新为新的B样条曲线
		Handle(Geom_TrimmedCurve) trimmedCurve = new Geom_TrimmedCurve(theInternalBSplineCurves[i], aSplitParamArray[0], aSplitParamArray[1]);
...
Standard_Boolean ArePointsEqual(const gp_Pnt& thePoint1, const gp_Pnt& thePoint2, Standard_Real theTolerance = 1e-6) {
	return thePoint1.Distance(thePoint2) <= theTolerance;
}

// 辅助函数：判断两个值是否在容差范围内相等
Standard_Boolean AreValuesEqual(Standard_Real theValue1, Standard_Real theValue2, Standard_Real theTolerance = 0.01) {
...
Standard_Boolean CurveOperate::isDegenerate(const Handle(Geom_BSplineCurve)& curve, Standard_Real toler) {
	...
	for (Standard_Integer i = poles.Lower() + 1; i <= poles.Upper(); ++i) {
		if (!poles(i).IsEqual(firstPole, toler)) {
			return Standard_False;
		}
	}
	return Standard_True;
...
std::tuple<std::vector<gp_Pnt>, std::vector<Standard_Real>> CurveOperate::CalCurvesInterPointsParamsToCurve(
	...
	for (const auto& curve : theCurves) {
		...
		try {
			...
			if (extrema.NbExtrema() > 0) {
				Standard_Boolean hasIntersection = Standard_False;
				...
				if (hasIntersection) {
					...
					// 没有交点，找到最近点
					Standard_Real U1, U2;
					Standard_Boolean hasNearest = extrema.TotalLowerDistanceParameters(U1, U2);
					if (hasNearest) {
...
Standard_Boolean CurveOperate::CompatibleWithInterPoints(const std::vector<Handle(Geom_BSplineCurve)>& theInterCurves, std::vector<Handle(Geom_BSplineCurve)>& theCompatibleCurves, Standard_Real theTolerance)
{
	...
	try {
		auto maxCurve = std::max_element(theCompatibleCurves.begin(), theCompatibleCurves.end(),
			[](const Handle(Geom_BSplineCurve)& curve1, const Handle(Geom_BSplineCurve)& curve2) -> Standard_Boolean {
				if (curve1.IsNull()) return Standard_True;
				if (curve2.IsNull()) return Standard_False;
...
Standard_Boolean CurveOperate::CompatibleWithInterPointsThree(const std::vector<Handle(Geom_BSplineCurve)>& theInterCurves, std::vector<Handle(Geom_BSplineCurve)>& theCompatibleCurves, Standard_Real theTolerance) {
	//0.检测theCompatibleCurves退化边
	if (theCompatibleCurves.size() <= 1) {
		return Standard_False;
	}

	Standard_Boolean firstDegenerate = Standard_False;
	Standard_Boolean lastDegenerate = Standard_False;
	...
	for (size_t i = 0; i < theCompatibleCurves.size(); ++i) {
		try {
			if (i == 0 && firstDegenerate) {
				...
				Standard_Integer degree = refCurve->Degree();
				...
				Standard_Integer numPoles = 0;
				for (Standard_Integer k = multiplicities.Lower(); k <= multiplicities.Upper(); ++k) {
				...
				for (Standard_Integer j = poles.Lower(); j <= poles.Upper(); ++j) {
...
Standard_Boolean MathTool::AreBSplinesCoPlanar(const std::vector<Handle(Geom_BSplineCurve)>& theCurves, const Standard_Real theToler)
{
	// 1. 收集所有控制点到一个容器
	std::vector<gp_Pnt> aPoleArray;
	aPoleArray.reserve(theCurves.size() * 10); // 预估容量，避免频繁realloc
	for (auto& aCurve : theCurves)
	{
		// 获取 B 样条曲线控制点
		const TColgp_Array1OfPnt& currentPoleArray = aCurve->Poles();
		// 将控制点插入到 allPoints 中
		for (Standard_Integer i = currentPoleArray.Lower(); i <= currentPoleArray.Upper(); ++i)
		{
			gp_Pnt currentPole = currentPoleArray.Value(i);
			// 这里可选择是否进行去重，比如用小哈希或近似坐标判断
			aPoleArray.emplace_back(currentPole);
		}
	}

	if (aPoleArray.size() < 3)
	{
		// 也可直接返回 Standard_True, 表示退化到1D或0D情况
		throw std::runtime_error("Not enough points to determine a plane.");
	}

	gp_Pnt aPoint0 = aPoleArray[0];
	Standard_Real maxDistance = INT_MIN;
	gp_Pnt aPoint1 = aPoint0;

	for (size_t i = 1; i < aPoleArray.size(); ++i)
	{
		Standard_Real aDistance = aPoint0.Distance(aPoleArray[i]);
		if (aDistance > maxDistance)
		{
			maxDistance = aDistance;
			aPoint1 = aPoleArray[i];
		}
	}

	if (maxDistance < Precision::Confusion())
	{
		// 或者 toler 也可以替代 Precision::Confusion()
		throw std::runtime_error("All points are coincident. Degenerate case.");
	}
	gp_Pnt aPoint2 = aPoint0;
	gp_Vec aVector01(aPoint0, aPoint1); // 向量 p1 - p0

	for (size_t i = 0; i < aPoleArray.size(); ++i)
	{
		if (aPoleArray[i].IsEqual(aPoint0, Precision::Confusion()) ||
			aPoleArray[i].IsEqual(aPoint1, Precision::Confusion()))
		{
			continue; // 不要和 p0 / p1 相同点重复计算
		}

		gp_Vec aVector02(aPoint0, aPoleArray[i]); // 向量 p2 - p0
		if (aVector01.Angle(aVector02) > theToler)
		{
			aPoint2 = aPoleArray[i];
		}
	}

	gp_Vec aVector02(aPoint0, aPoint2);
	gp_Vec normalVec = aVector01 ^ aVector02;
	normalVec.Normalize(); // 单位化

	for (const auto& aPnt : aPoleArray)
	{
		gp_Vec aVector0p(aPoint0, aPnt);
		if (aVector0p.Magnitude() == 0) continue;
		Standard_Real anAngle = aVector0p.Angle(normalVec) * 180 / M_PI;
		if (std::abs(anAngle - 90) > theToler)
		{
			return Standard_False;
...
Standard_Real MathTool::ComputeCurveLengthBetweenParameters(const Handle(Geom_BSplineCurve)& theCurve, gp_Pnt thePnt1, gp_Pnt thePnt2)
{
...
Standard_Real MathTool::GetPntParameterOnCurve(const Handle(Geom_BSplineCurve)& theCurve, const gp_Pnt& thePoint)
{
	// 创建投影工具
...
Standard_Real MathTool::ComputeCurveCurvature(const Handle(Geom_Curve)& curve, Standard_Real parameter)
{
	gp_Pnt point;
	gp_Vec firstDerivative, secondDerivative;

	// 计算一阶和二阶导数
	curve->D2(parameter, point, firstDerivative, secondDerivative);

	// 计算一阶导数的模
	Standard_Real firstDerivativeMagnitude = firstDerivative.Magnitude();

	// 计算一阶导数和二阶导数的叉积
	gp_Vec crossProduct = firstDerivative.Crossed(secondDerivative);

	// 计算曲率
	Standard_Real curvature = crossProduct.Magnitude() / (firstDerivativeMagnitude * firstDerivativeMagnitude * firstDerivativeMagnitude);

	return curvature;
}

Standard_Real MathTool::ComputeCurveCurvatureDerivative(const Handle(Geom_BSplineCurve)& curve, Standard_Real parameter)
{
	const Standard_Real epsilon = 1e-12;
	gp_Pnt P;
	gp_Vec D1, D2, D3;
	curve->D3(parameter, P, D1, D2, D3);

	// 计算C'和C''的叉乘及其模长
	gp_Vec D1CrossD2 = D1.Crossed(D2);
	Standard_Real A = D1CrossD2.Magnitude();

	// 处理曲率为零的情况
	if (A <= epsilon)
		return 0.0; // 曲率接近于零，曲率变化率无意义

	// 计算C'和C'''的叉乘
	gp_Vec D1CrossD3 = D1.Crossed(D3);

	// 计算分子部分：(C'×C''')·(C'×C'')
	Standard_Real numeratorPart1 = D1CrossD3.Dot(D1CrossD2);
	Standard_Real A_prime = numeratorPart1 / A;

	// 计算|C'|及其模长的立方
	Standard_Real C1_mag = D1.Magnitude();
	if (C1_mag <= epsilon)
		return 0.0; // 速度接近于零，曲率变化率无意义

	Standard_Real B = std::pow(C1_mag, 3);

	// 计算B' = 3 * |C'| * (C'·C'')
	Standard_Real C1_dot_C2 = D1.Dot(D2);
	Standard_Real B_prime = 3.0 * C1_mag * C1_dot_C2;

	// 曲率变化率公式
	Standard_Real dkdu = (A_prime * B - A * B_prime) / (B * B);
...
Handle(Geom_BSplineCurve) MathTool::CreateCircleApproximation(Standard_Real radius)
{
	// 定义控制点数量
	const Standard_Integer numControlPoints = 9; // 使用9个控制点
	TColgp_Array1OfPnt controlPoints(1, numControlPoints);
	...
	;
	std::vector<Standard_Real> params = ComputeUniformParam(OccArrayConvertoVector(controlPoints).size(), 0., 1.);
	std::vector<Standard_Real> tempKnots = KnotGernerationByParams(params, 3, 3);
	std::vector<Standard_Real> insertKnots;

	Handle(Geom_BSplineCurve) aBSplineCurve = IterateApproximate(insertKnots, OccArrayConvertoVector(controlPoints), params, tempKnots, 3, 50, 0.01);
	return aBSplineCurve;
}
Standard_Real MathTool::ComputeCurvatureDerivative(const Handle(Geom_Curve)& curve, Standard_Real parameter)
{
	gp_Pnt point;
	gp_Vec firstDerivative, secondDerivative, thirdDerivative;

	// 计算一阶、二阶和三阶导数
	curve->D3(parameter, point, firstDerivative, secondDerivative, thirdDerivative);

	// 计算一阶导数的模
	Standard_Real firstDerivativeMagnitude = firstDerivative.Magnitude();

	// 计算一阶导数和二阶导数的叉积
	gp_Vec crossProduct1 = firstDerivative.Crossed(secondDerivative);

	// 计算一阶导数和三阶导数的叉积
	gp_Vec crossProduct2 = firstDerivative.Crossed(thirdDerivative);

	// 计算曲率的变化率
	Standard_Real curvatureDerivative = (crossProduct1.Dot(crossProduct2)) / (firstDerivativeMagnitude * firstDerivativeMagnitude * firstDerivativeMagnitude * firstDerivativeMagnitude);

	return curvatureDerivative;
}
Standard_Real MathTool::GetParameterAtLength(const Handle(Geom_BSplineCurve)& theCurve, Standard_Real theStartParam, Standard_Real theTargetLength, Standard_Real theTolerance)
{
	// 检查起始参数是否在曲线范围内
	if (theStartParam < theCurve->FirstParameter() || theStartParam > theCurve->LastParameter())
	{
		throw std::invalid_argument("Start parameter is out of curve range.");
	}

	// 检查目标长度是否超过从起点到曲线终点的长度
	Standard_Real maxLength = ComputeCurveLengthBetweenParameters(theCurve, theStartParam, theCurve->LastParameter());
	if (theTargetLength >= maxLength)
	{
		return theCurve->LastParameter(); // 返回曲线末尾参数
	}

	// 初始化参数
	Standard_Real low = theStartParam;
	Standard_Real high = theCurve->LastParameter();
	Standard_Real mid;
	Standard_Real curveLength;
	Standard_Integer maxIterations = 1000;
	Standard_Integer iterationCount = 0;

	// 二分法搜索
	while (iterationCount < maxIterations)
	{
		mid = (low + high) / 2; // 取中间参数
		curveLength = ComputeCurveLengthBetweenParameters(theCurve, theStartParam, mid);

		// 检查收敛条件
		if (std::abs(curveLength - theTargetLength) <= theTolerance)
		{
			return mid;
		}

		// 根据长度调整搜索范围
		if (curveLength > theTargetLength)
		{
			high = mid; // 缩小到左半区
		}
		else
	...
	throw std::runtime_error("Iteration limit exceeded. Target length may not be reachable.");
}